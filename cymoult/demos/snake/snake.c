...
#define UPKEY SDLK_UP /* see http://www.libsdl.org/docs/html/sdlkey.html for list */
...
// /Global variables    
const char * imagenames[NUMIMAGES]= {"backdrop.gif","text.gif","snake.gif","rawbackdrop.gif"};  
...
long numImages,numSprites,atefood,score;
...
long fps,tickcount,lasttick,lastfruittick, fruitdelay;
char buffer[50];
long snakespeedticks,snakemoved,snakemovedir; // dir 0=N,1=E,2=S,3=W
...
void InitGame() {
    ...
    screen = SDL_SetVideoMode(17*GRWIDTH,16*GRHEIGHT , 32, SDL_SWSURFACE | SDL_DOUBLEBUF  );    
    fruittrapcount=0;
    paused=0;
    showfps=0;
    framecount=0;
    snakemovedir=0;
//    snakespeedticks=10250; // ms per move
    snakespeedticks=1250; // ms per move
    fruitdelay=1500;
...
void RandomFruitTrap() {
    ...
    int foundPlace=0;
    int thing;
    if (SDL_GetTicks()-lastfruittick < fruitdelay)
        return;
    lastfruittick = SDL_GetTicks();
